...
-(void)setOriginFrame
{
    self.originFrame=CGRectMake(CGRectGetMinX(self.frame), CGRectGetMinY(self.frame), CGRectGetWidth(self.frame), CGRectGetHeight(self.frame));
}
...
//- (UIImage *)captureViewAndClipRect:(CGRect)clipRect
//{
...
//    UIGraphicsEndImageContext();
//    
//	UIImage *cropped = [capturedImage croppedImage:clipRect];
//    
//    return cropped;
//}


- (UIImage *)captureScrollviewFromScreen:(UIScrollView *)scrollView
{
    UIImage* image = nil;
      ...
      shrinkSpacingToFit:(BOOL)shrinkSpacingToFit
{
    ...
    __block NSInteger numVisibleViews = 0;
...
- (void)addBorder:(BorderPosttion)position color:(UIColor *)color height:(CGFloat)height
{
    
    //为了在retina屏中创建0.5个点的边框，非retina屏要 * 2 ,因为0.5在此屏中创建不出来
    CGFloat calculateHeight = height;
    if (height == 0.5) {
    ...
    if (position & BorderTop) {
        
        CGRect frame = UIEdgeInsetsInsetRect(self.bounds, UIEdgeInsetsMake(0 , 0, self.frame.size.height - calculateHeight, 0));
        [self addLayerWithFrame:frame backgroundColor:color];
    }
    
    if (position & BorderBottom) {
        
        CGRect frame = UIEdgeInsetsInsetRect(self.bounds, UIEdgeInsetsMake(self.frame.size.height - calculateHeight , 0, 0, 0));
        [self addLayerWithFrame:frame backgroundColor:color];
    }

}

- (void)addLayerWithFrame:(CGRect)frame backgroundColor:(UIColor *)bgColor
{
