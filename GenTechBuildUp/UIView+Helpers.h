...
//边框位置
typedef NS_ENUM(NSInteger, BorderPosttion){
    BorderTop = 1 << 0,
    BorderLeft = 1 << 1,
    BorderBottom = 1 << 2,
    BorderRight = 1 << 3,
...
- (void)bottomAlignForView:(UIView *)view;  //底部与view的顶部对齐
...
//- (UIImage *)captureViewAndClipRect:(CGRect)clipRect;  //截图并按clipRect 进行截剪
...
+ (UIView *)createOnePXHeightViewWithFrame:(CGRect)frame bgColor:(UIColor *)color;  //创建一个高度为1px的view
